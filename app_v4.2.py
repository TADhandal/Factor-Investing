...
def main():
    ...
    start_date = st.sidebar.date_input('**Start Date**', value=pd.to_datetime('today') - dt.timedelta(days=1825), format="DD/MM/YYYY", min_value=dt.datetime(2005, 1, 1))
    end_date = st.sidebar.date_input('**End Date**', format="DD/MM/YYYY", min_value=start_date, max_value=pd.to_datetime('today'))
        ...
        with tab2:
            ...
            for index, df in data_frames.items():
                ...
                fig_RR.update_layout(
                    title=f"Rolling Returns for {index}",
                    xaxis_title="Date",
